# read in a BMP file palette (c) 2013 @whaleygeek
...
class BMP():
  ...
  def drawPixels(self, mc, sx, sy, sz):
    
    set = GLOWING_OBSIDIAN
    clear = AIR
    ...
    for rowno in range(rows):
      ...
      for byteno in range(rowsize):
        ...
        for bit in range(8):
          if (byte & mask) != 0:
            rowstr += " "
            mc.setBlock(x, y, z, clear)
          else:
            rowstr += "x"
            mc.setBlock(x, y, z, set)
          mask >>= 1
          bitcount = bitcount + 1
          x = x + 1
          if bitcount >= self.dib_width:
...
if __name__ == '__main__':
  ...
  if bmp.dib_bpp != 1:
    ...
    bmp.drawPixels(mc, pos.x, pos.y+2, pos.z)
